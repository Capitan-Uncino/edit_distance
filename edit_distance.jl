...
function apply_errors(strings::Vector{String},
    probs::Dict{Char,Dict{Char,Float64}})
    ...
    prob_insert = 0.05
    prob_delete = 0.05
    prob_error   = prob_swap + prob_insert + prob_delete
    ...
    for (i, s) in enumerate(strings)
        ...
        while j ≤ length(chars)
            c = chars[j]
            error_type = rand(Categorical([prob_swap, prob_insert, prob_delete, 1 - prob_error]))

            if error_type == 1   # swap
                if haskey(probs, c)
            ...
            elseif error_type == 2   # insert
                letters = collect(keys(probs[c]))
                p = [probs[c][l] for l in letters]
                insert!(chars, j, letters[rand(Categorical(p))])
                j += 2 

            elseif error_type == 3   # delete
                deleteat!(chars, j)
